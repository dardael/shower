...
export interface ISendAppointmentAdminConfirmation {
  execute(input: BaseEmailInput): Promise<boolean>;
}

@injectable()
export class SendAppointmentAdminConfirmation
  extends BaseAppointmentEmailUseCase
  implements ISendAppointmentAdminConfirmation
{
  constructor(
  ...
  protected getTemplateType(): EmailTemplateType {
    return 'appointment-admin-confirmation';
  }
  ...
  protected getLogMessage(): string {
    return "Échec de l'envoi de l'email de confirmation administrateur";
  }
}
